...
class AccessibilityWorkflow:
    """LangGraph workflow for accessibility testing and fixing."""
    ...
    def _build_workflow(self) :
        ...
        # Add nodes
        workflow.add_node("clone_repo", self.clone_repo)
        workflow.add_node("analyze_accessibility", self.analyze_accessibility)
        workflow.add_node("locate_issues", self.locate_issues)
        workflow.add_node("delegate_fixes", self.delegate_fixes)
        workflow.add_node("critique_solutions", self.critique_solutions)
        workflow.add_node("apply_and_push", self.apply_and_push)
        ...
        workflow.add_edge("critique_solutions", "apply_and_push")
    ...
    def clone_repo(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            repo_path = self.github_handler.clone_repository(repo_url)
            source_files = self.github_handler.get_source_files(repo_path)
    ...
    def analyze_accessibility(self, state: AgentState) -> AgentState:
    ...
    def locate_issues(self, state: AgentState) -> AgentState:
    ...
    def delegate_fixes(self, state: AgentState) -> AgentState:
    ...
    def critique_solutions(self, state: AgentState) -> AgentState:
    ...
    def apply_and_push(self, state: AgentState) -> AgentState:
    ...
    def generate_reports(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            accessibility_report = self.report_generator.generate_accessibility_report(
                state['issues'],
                state['repo_name']
            )
            ...
            fix_report = self.report_generator.generate_fix_report(
                fixes_info,
                state['repo_name']
            )
            
            # Generate critique report
            critique_report = self.report_generator.generate_critique_report(
                state.get('critiques', []),
                state['repo_name']
            )
    ...
    def run(self, repo_url: str, repo_name: str) -> AgentState:
        """
        Run the complete workflow.
        
        Args:
            repo_url: URL of the GitHub repository
            repo_name: Name of the repository (user/repo)
